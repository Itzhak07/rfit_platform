...
// GET All Workouts by Client ID
router.get("/client", auth, async function (req, res, next) {
  const { id } = req.user;

  try {
    const workouts = await WorkoutsController.getClientWorkouts(id);
...
router.post(
  ...
  [
    ...
    check("notes", "Workout is required!").not().isEmpty(),
    check("date", "Date is required!").not().isEmpty(),
    check("startDate", "Starting Time is required!").not().isEmpty(),
    check("endDate", "Ending Time is required!").not().isEmpty(),
  ],
  auth,
